class BotMessage:
    ...
    TEMPERATURE_TEXT = """Что такое точность генерации?\n\nТочность генерации - это значение от 0 до 10, 
        которое позволяет вам контролировать, насколько уверенной должна быть модель, делая\nэти предсказания. Снижение 
        точности означает, что модель будет меньше рисковать, и завершения \nбудут более точными и детерминированными. 
        \nПовышение значения приведет к более разнообразным завершениям.\n\nЗначение a 0.6 является хорошим значением по 
        умолчанию для получения уникальных результатов по модели"""
